...
//			return false;
			...
			recordingManager.setPreviewSize((Size) sizes.get(0));
			return false;
		}
		
		@Override
		public Size getVideoSize() {
			Log.i("VIDEO SIZE", videoSize.width + " video "+videoSize.height);
			return videoSize;
		}
		...
		if (Build.VERSION.SDK_INT >= 11) {
			...
			videoSizeSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
					Log.i("VideoRecording", " itemselected "+ arg2);
			...
			for(int i=0; i< videoSizeSpinner.getCount(); i++){
				Log.i("SIZE", "size "+ ((Size) videoSizeSpinner.getItemAtPosition(i)).height + " " + ((Size) videoSizeSpinner.getItemAtPosition(i) ).width);
			}
			videoSize = (Size) videoSizeSpinner.getItemAtPosition(0);
		
			recordingManager.setPreviewSize(videoSize);
			Log.i("TEST123", "video size "+ videoSize.height + " " + videoSize.width);
		}
		else {
